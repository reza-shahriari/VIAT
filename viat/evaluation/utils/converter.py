...
try:
    import viat.evaluation.utils.general_utils as general_utils
    import viat.evaluation.utils.validations as validations
    from viat.evaluation.bounding_box import BoundingBox
    from viat.evaluation.utils.enumerators import BBFormat, BBType, CoordinatesType
except ImportError:
...
def coco2bb(path, bb_type=BBType.GROUND_TRUTH,is_obb=False):
    ret = []
    ...
    for file_path in annotation_files:
        ...
        if 'categories' in json_object:
            for c in json_object['categories']:
                ...
                classes[c_id] = c_name
                try:
                    classes[str(c_id)] = c_name
                except Exception:
                    pass
                try:
                    classes[int(c_id)] = c_name
                except Exception:
                    pass

        images = {}
        for i in json_object.get('images', []):
            i_info = {
                'file_name': i.get('file_name', f"image_{i.get('id', 0)}"),
                'img_size': (int(i.get('width', 0)), int(i.get('height', 0)))
            }
            images[i.get('id')] = i_info
            try:
                images[str(i.get('id'))] = i_info
            except Exception:
                pass
            try:
                images[int(i.get('id'))] = i_info
            except Exception:
                pass

        annotations = json_object.get('annotations', [])
        ...
        for annotation in annotations:
            ...
            if not is_obb:
                ...
                if len(bbox) >= 4:
                    x1, y1, bb_width, bb_height = bbox[:4]
                else:
                    continue
            else:
            ...
            if bb_type == BBType.DETECTED and 'score' not in annotation.keys():
                ...
                return ret
            confidence = annotation.get('score') if bb_type == BBType.DETECTED else None

            # Make image name only the filename, without extension
            img_info = images.get(img_id, {'file_name': f'image_{img_id}', 'img_size': (0, 0)})
            img_name = general_utils.get_file_name_only(img_info['file_name'])

            # Safely resolve class_id without KeyError
            raw_cat_id = annotation.get('category_id')
            if raw_cat_id in classes:
                class_id = classes[raw_cat_id]
            elif str(raw_cat_id) in classes:
                class_id = classes[str(raw_cat_id)]
            elif isinstance(raw_cat_id, (int, float)) and int(raw_cat_id) in classes:
                class_id = classes[int(raw_cat_id)]
            elif isinstance(raw_cat_id, (int, float)) and (int(raw_cat_id) + 1) in classes:
                class_id = classes[int(raw_cat_id) + 1]
            elif isinstance(raw_cat_id, (int, float)) and (int(raw_cat_id) - 1) in classes:
                class_id = classes[int(raw_cat_id) - 1]
            elif len(classes) == 1:
                class_id = next(iter(classes.values()))
            elif raw_cat_id is not None:
                class_id = str(raw_cat_id)
            else:
                class_id = 'Object'
...
def openimage2bb(annotations_path, images_dir, bb_type=BBType.GROUND_TRUTH):
    if pd is None:
        raise ImportError("The 'pandas' package is required for OpenImage dataset format. Please install it: pip install pandas")
    ret = []
    # Get annotation files in the path
...
def xml2csv(xml_path):
    ...
    if pd is None:
        raise ImportError("The 'pandas' package is required for XML to DataFrame conversion. Please install it: pip install pandas")
    xml_list = []
    xml_df = pd.DataFrame()
    try:

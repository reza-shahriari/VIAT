...
try:
    from viat.evaluation.utils.enumerators import CoordinatesType, BBFormat, BBType, FileFormat
    from viat.evaluation.utils.general_utils import get_files_recursively
except ImportError:
    from .enumerators import CoordinatesType, BBFormat, BBType, FileFormat
...
def csv_contains_columns(file_path, columns, sep=','):
    ...
    try:
        with open(file_path, 'r', newline='', encoding='utf-8', errors='ignore') as f:
            ...
            header = next(reader, [])
        cols_1 = [col.strip().lower() for col in header]
        cols_2 = [col.strip().lower() for col in columns]
        cols_1.sort()
        cols_2.sort()

...
try:
    from viat.evaluation.utils.general_utils import (convert_to_absolute_values,
...
class BoundingBox:
    ...
    def set_coordinates(self, coordinates, type_coordinates, img_size=None):
        ...
        # For absolute coords: (x,y,w,h)=real bb coords
            ...
            elif self._format == BBFormat.XYX2Y2:  # self._format == BBFormat.XYX2Y2: <left> <top> <right> <bottom>.
                self._x2 = coordinates[2]
                self._y2 = coordinates[3]
                self._w = self._x2 - self._x
                self._h = self._y2 - self._y
            elif self._format == BBFormat.OBB: # self._format == BBFormat.OBB: <x1> <y1> <x2> <y2> <x3> <y3> <x4> <y4>.
                self._x1 = coordinates[0]
                self._y1 = coordinates[1]
                self._x2 = coordinates[2]
                self._y2 = coordinates[3]
                self._x3 = coordinates[4]
                self._y3 = coordinates[5]
                self._x4 = coordinates[6]
                self._y4 = coordinates[7]
            
        # Convert all values to float
        if not self._format == BBFormat.OBB:
            ...
            self._x1 = float(self._x1) 
            self._y1 = float(self._y1) 
            self._x2 = float(self._x2) 
            self._y2 = float(self._y2) 
            self._x3 = float(self._x3) 
            self._y3 = float(self._y3) 
            self._x4 = float(self._x4) 
            self._y4 = float(self._y4) 
    ...
    def get_absolute_bounding_box(self, format=BBFormat.XYWH):
        ...
        elif format ==BBFormat.OBB:
            return (self._x1, self._y1, self._x2, self._y2, self._x3, self._y3, self._x4, self._y4)
    ...
    def get_area(self):
        if self._format!= BBFormat.OBB:
            ...
            area = 0.5 * abs(self._x1*self._y2 + self._x2*self._y3 + self._x3*self._y4 + self._x4*self._y1 - 
                             (self._y1*self._x2 + self._y2*self._x3 + self._y3*self._x4 + self._y4*self._x1))
            return area
    ...
    def __str__(self):
        ...
        elif self._format == BBFormat.OBB:
            abs_bb_obb = self.get_absolute_bounding_box(format=BBFormat.OBB)
            area = self.get_area()
            return f'image name: {self._image_name}\nclass: {self._class_id}\nbb (OBB): {abs_bb_obb}\narea: {area}\nbb_type: {self._bb_type}'

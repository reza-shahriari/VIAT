...
import os
import numpy as np
from ..utils import linear_sum_assignment
...
class HOTA(_BaseMetric):
    ...
    def plot_single_tracker_results(self, table_res, tracker, cls, output_folder,stadium_download=''):
        ...
        plt.xlabel('alpha')
        plt.ylabel('score')
        plt.title('tracker resualt')
        plt.axis([0, 1, 0, 1])
        legend = []
        for name in self.float_array_fields:
            legend += [name + ' (' + str(np.round(np.mean(res[name]), 2)) + ')']
        plt.legend(legend, loc='lower left')
        out_file = os.path.join(output_folder, 'Tracker'+stadium_download+'_plot.pdf')

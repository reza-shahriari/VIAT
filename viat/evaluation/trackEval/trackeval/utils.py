...
def write_summary_results(summaries, cls, output_folder,stadium_download = ''):
    ...
    out_file = os.path.join(output_folder,'eval_tracker_summary'+stadium_download+'.csv')
    os.makedirs(os.path.dirname(out_file), exist_ok=True)
    
    # print('wrting into'+output_folder + 'eval_tracker_summary.csv file' )
    with open(out_file, 'w', newline='') as f:
        writer = csv.writer(f)
        # print('file opend!')
        writer.writerow(fields)
        writer.writerow(values)
    # print('eval_tracker_summary is creted ')

def write_detailed_results(details, cls, output_folder,stadium_download = ''):
    """Write detailed results to file"""
    sequences = details[0].keys()
    fields = ['seq'] + sum([list(s['COMBINED_SEQ'].keys()) for s in details], [])
    out_file = os.path.join(output_folder, 'eval_tracker_detailed' + stadium_download + '.csv')
    os.makedirs(os.path.dirname(out_file), exist_ok=True)
    # print('wrting into'+output_folder + 'eval_tracker_detailed.csv file' )
    with open(out_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(fields)

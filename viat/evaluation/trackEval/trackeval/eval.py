...
try:
    from viat.evaluation.conf.configs import config as my_configs
except ImportError:
    try:
        from conf.configs import config as my_configs
    except ImportError:
        my_configs = None
try:
    import tqdm
...
class Evaluator:
    ...
    def __init__(self, config=None,path_obeject=None):
        ...
        for dataset, dataset_name in zip(dataset_list, dataset_names):
            ...
            for tracker in tracker_list:
                ...
                try:
                    ...
                    output_fol = output_fol.split('/')[:-4]
                    output_fol = '/'.join(x for x in output_fol) +'/evaluation_result/'
                    output_fol = self.path_object.det_path + '/evaluation_result'
                    stadium_download = self.path_object.stadium_downloads.pop() if len(self.path_object.stadium_downloads) else ''
                            ...
                            for metric, metric_name in zip(metrics_list, metric_names):
                                ...
                                if config['PLOT_CURVES']:
                                    metric.plot_single_tracker_results(table_res, tracker_display_name, c_cls,
                                                                       output_fol,stadium_download)
                            if config['OUTPUT_SUMMARY']:
                                
                                utils.write_summary_results(summaries, c_cls, output_fol,stadium_download)
                            if config['OUTPUT_DETAILED']:
                                utils.write_detailed_results(details, c_cls, output_fol,stadium_download)

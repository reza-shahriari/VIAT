...
def detect_edges(frame, rect, edge_type):
    ...
    size_factor = min(w, h)
    padding = max(5, min(20, int(size_factor * 0.2)))  # Between 5 and 20 pixels, scaled by object size
    
    # Define region of interest based on edge type
    if edge_type == 'top':
        roi = frame[max(0, y-padding):min(frame.shape[0], y+padding), 
                   max(0, x):min(frame.shape[1], x+w)]
        axis = 0  # y-axis
    elif edge_type == 'bottom':
        roi = frame[max(0, y+h-padding):min(frame.shape[0], y+h+padding), 
                   max(0, x):min(frame.shape[1], x+w)]
        axis = 0  # y-axis
    elif edge_type == 'left':

...
def import_segmentation_masks(
    ...
    for i, rgb_path in enumerate(rgb_files):
        ...
        if len(mask_img.shape) == 2:
            ...
            # convert to BGR equivalents for processing
            unique_colors_bgr = [(val, val, val) for val in unique_colors]
            mask_img_bgr = cv2.cvtColor(mask_img, cv2.COLOR_GRAY2BGR)
        else:
            # BGR mask
            mask_img_bgr = mask_img[:, :, :3] # ignore alpha if present
            # Reshape to a list of pixels
            pixels = mask_img_bgr.reshape(-1, 3)
            unique_colors_bgr = np.unique(pixels, axis=0)
            unique_colors_bgr = [tuple(color) for color in unique_colors_bgr]
